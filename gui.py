    ...
    def mouseReleaseEvent(self,event):
        if self.openFlag == 1:
            ...
            if self.moveFlag == 0:
                ...
                self.setPixmap(temp)
                print("(x0,y0,x1,y1) = null")
            else:
                print("(x0,y0,x1,y1) = (" + str(self.x0) + "," + str(self.y0) + "," + str(self.x1) + "," + str(self.y1) + ")")
    #鼠标移动事件
...
class Ui_MainWindow(object):
    ...
    def slot_save(self):
        if self.path_flag:
            path = self.label_path.text()
            jpg = cv2.imread("img_tempout/cover.jpg")
            index=path.find("/imgs/")
            jpg_name = path[index+6:-4]
            jpg_prefix = path[0:index+1]
            # print(path.find("/imgs/"))
