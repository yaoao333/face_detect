...
class mtcnn():
    ...
    def detectFace(self, img, threshold):
        ...
        #cv2.imshow("111", copy_img)
        origin_h, origin_w, _ = copy_img.shape
        # -----------------------------#
        #   计算原始输入图像
        #   每一次缩放的比例
        # -----------------------------#
        scales = funcs_mtcnn.calculateScales(img)
        ...
        for scale in scales:
            ...
            out.append(ouput)
        # out用于保存图像金字塔每张图片的Pnet预测结果
        rectangles = []

        # -------------------------------------------------#
        #   对图像金字塔的预测结果进行循环
        #   取出每张图片的种类预测和回归预测结果
        #   由于Pnet会对输入的图片进行压缩，例如第一次scale所得的图片大小为870*500 而经过Pnet处理后该图片尺寸会变为440*245
        # -------------------------------------------------#
        ...
        for i in range(len(scales)):
            ...
            # -------------------------------------#
            #   解码的过程
            #   rectangle代表金字塔每层图片的所有预测框，而rectang存les保所有层
            # -------------------------------------#
            rectangle = funcs_mtcnn.detect_face_12net(cls_prob, roi, out_side, 1 / scales[i], origin_w, origin_h,
                                                threshold[0], i)
            rectangles.extend(rectangle)

            # print(np.shape(rectangle))

        # -------------------------------------#
        #   非极大抑制
        # -------------------------------------#
        rectangles = np.array(funcs_mtcnn.NMS(rectangles, 0.7))
        ...
        #   解码的过程
        # -------------------------------------#
        rectangles = funcs_mtcnn.filter_face_24net(cls_prob, roi_prob, rectangles, origin_w, origin_h, threshold[1])
        ...
        #   解码的过程
        # -------------------------------------#
        rectangles = funcs_mtcnn.filter_face_48net(cls_prob, roi_prob, pts_prob, rectangles, origin_w, origin_h, threshold[2])
